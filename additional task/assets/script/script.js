...
const products = document.querySelectorAll('.item__price') // Получаем массив элементов
const totalPriceElement = document.querySelector('.total')
const updateTotalPrice = () => {
  let totalPrice = 0;
  products.forEach(product => {
    totalPrice += +product.innerText
    totalPriceElement.innerHTML = totalPrice
  })
}
const updatePrices = () => {
  products.forEach(product => {
    product.classList.toggle('discount-price')
    if(product.classList.contains('discount-price')) {
      product.innerHTML = +product.innerText * 0.8
    }else {
      product.innerHTML = +product.innerText * 1.25 // Данная строчка возвращает старые значения
    }
  })
  updateTotalPrice()
}
updateTotalPrice()
document.querySelector('.btn').onclick = updatePrices